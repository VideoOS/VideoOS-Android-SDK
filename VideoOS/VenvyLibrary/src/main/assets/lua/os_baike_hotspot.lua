...
local function setTitleBgSize(titleBg, title)
    ...
    if (System.ios()) then
        title:frame(20 * scale, 0, baike.labelWidth + 10 * scale, h)
...
local function createBaikeTitle(data)
    ...
    if (System.android()) then
        titleBg:xy(20 * scale, 0)
        titleBg:align(Align.V_CENTER)
        title:margin(25 * scale, 7 * scale, 10 * scale, 7 * scale)
    else
...
local function onCreate(data)
    ...
    if System.ios() then
        local textWidth, textHeight = Native:stringSizeWithWidth(data.data.hotEditInfor.hotTitle, 1000, 16)
        baike.labelWidth = textWidth
    end
    ...
    if System.ios() then
        ...
        if (isPortrait) then
            baike.titleBg:translation(-20 * scale, 0)
