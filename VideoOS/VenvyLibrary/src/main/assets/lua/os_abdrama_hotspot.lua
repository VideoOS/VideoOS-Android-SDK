...
local function closeView()
    ...
    if Native:getCacheData(abdrama.id) == tostring(eventTypeShow) then
        Native:widgetEvent(eventTypeClose, abdrama.id, adTypeName, actionTypeNone, "")
        Native:deleteBatchCacheData({ abdrama.id })
    end
    Native:destroyView()
end

local function onClickDramaLink(linkUrl)
    -- body
    Native:widgetEvent(eventTypeClick, abdrama.id, adTypeName, actionTypeOpenUrl, linkUrl)
...
local function createDramaOptionsView(data, isPortrait)
    ...
    dramaOptionAView:onClick(function()
        -- body
        onClickDramaLink(data.data.hotEdit.alinkVideoId)
    end)

    dramaOptionBView:onClick(function()
        -- body
        onClickDramaLink(data.data.hotEdit.blinkVideoId)
    end)
...
local function setDramaTime()
    ...
    countDownTimer:callback(function()
        -- body
        onClickDramaLink(abdrama.data.data.hotEdit.alinkVideoId)
    end)
...
function show(args)
    ...
    Native:widgetEvent(eventTypeShow, abdrama.id, adTypeName, actionTypePauseVideo, "")
