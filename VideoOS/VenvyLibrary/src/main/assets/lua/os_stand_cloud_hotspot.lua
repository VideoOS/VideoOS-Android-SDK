...
local OS_IMAGE_TEST = "http://videojj-mobile.oss-cn-beijing.aliyuncs.com/mock/os/dream_ad.jpg"
...
function onCreate(dataTable)
    ...
    cloud.cloudImage:onClick(function()
        local linkUrl = getLinkUrl(dataTable)
        if (linkUrl == nil) then
            return
        end
        Native:widgetEvent(eventTypeClose, cloud.id, adTypeName, actionTypeOpenUrl, linkUrl)
        closeView()
        -- 点击统计
