...
import static cn.com.venvy.common.media.Preconditions.checkArgument;
...
class Pinger {
    ...
    boolean ping(int maxAttempts, int startTimeout) {
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                VenvyLog.e("Error pinging server (attempt: " + attempts + ", timeout: " + timeout + "). ");
            } catch (InterruptedException | ExecutionException e) {
                VenvyLog.e("Error pinging server due to unexpected error", e);
            }
            attempts++;
            timeout *= 2;
        }
        ...
        VenvyLog.e(error, new ProxyCacheException(error));
        return false;
    }
    ...
    private boolean pingServer() throws ProxyCacheException {
        ...
        try {
            ...
            VenvyLog.i("Ping response: `" + new String(response) + "`, pinged? " + pingOk);
            return pingOk;
        } catch (ProxyCacheException e) {
            VenvyLog.e("Error reading ping response", e);
            return false;
        } finally {
            source.close();
        }
